...
Use the AskQuestion tool (or ask the user directly) to choose depth:
...
Present these as clear options so the user can pick one.

## Step 3: Question Flow

Ask questions to fill gaps and add product-level context. Use AskQuestion when possible (with 2–4 options where helpful). Map answers to the project-context structure:
...
- They can edit the file anytime or re-run this command (e.g. with a new PRD) to update context.
